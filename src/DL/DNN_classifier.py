...
from tensorflow.keras.layers import Embedding,Dense, Activation, MaxPool1D, LSTM, Conv1D, Flatten, Dropout
...
flags.DEFINE_integer('maxlen', 164, 'maximum number of words to keep in one tweet')
...
def load_csv_and_embedding(train_path, validation_path, embedding_path, emb_dim, num_words = 10000, maxlen = 140, partial = True):
    ...
    if partial == True:
        ...
        train_partial_idx = int(0.1 * len(train_tweets))
        ...
        val_partial_idx = int(0.1*len(val_tweets))
...
class MLP_classifier():
    def __init__(self, emb_mat, maxlen, hidden_layers = [64, 32, 16], learning_rate = 1e-4, trainable=True):
        self.emb_mat = emb_mat
        self.vocab = emb_mat.shape[0]
        self.emb_dim = emb_mat.shape[1]
        self.maxlen = maxlen
    ...
    def create_model(self):
        ...
        # Embedding Layer
        model.add(Embedding(input_dim=self.vocab, output_dim=self.emb_dim, input_length=self.maxlen, 
                            weights=[self.emb_mat], trainable=self.trainable))
...
def main(_):
	...
	if 	FLAGS.model == 'CNN':
    	...
    	model = MLP_classifier(emb_mat, FLAGS.maxlen).model

	print(model.summary()) 

	history = model.fit(x_train, y_train, epochs = FLAGS.epoch, verbose=True, batch_size=FLAGS.batch_size, callbacks = [BCP()], validation_data = (x_val, y_val))
	...
	np.savez(FLAGS.model +'_training_history.npz', loss = BCP.batch_loss, accuracy = BCP.batch_accuracy)
